...
1. Все стили, применяемые к HTML элеметнам обрабатываются генератором при создании компонента. В связи с этим все view функции, в которых у HTML элементов определяется `style` должны быть объявлены в одном файле с компонентом, к которому эта view функция относится.
