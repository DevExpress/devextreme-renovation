...
Так же возвращаемая разметка из `view` должна содержать 1 рутовый элемент. Если Вам необходимо вернуть несколько, оберните их в специальный эелемент `Fragment` (см. [Use cases](#JSX))
...
Для подготовки (компиляции) компонентов к тестированию, необходимо воспользоваться тасками `test-env`, `dev`, либо gulp таской `generate-components`. Рядом с вашим декларативным компонентом появится файл с расширением .p.js. (Это Preact компонент, сгенеренный из декларации. Пока что он используется для тестирования)
...
`TwoWay` пропы делаеют все то же самое, что и `OneWay`, предоставляют компоненту входные параметры. 
...
Так же пользователь Вашего компонента получит оповещение, что значение этого пропа сменилось. В Реакте можно будет подписаться на специально созданный ивент, в Ангуляре можно будет настроить двусторонний байндинг и т.п.
...
А в каких случаях надо их объявить? Если Вы создаете компонент (A), который хочет подписаться на Change ивент другого компонента (B), то у компонента (B) должен быть объявлен необходимый `Event`. Это нужно только для корректной передачи пропов в декларации, на сгенерированный код это никак не влияет. 
...
**Еще раз. Наличие или отсуствие этих пропов в декларации не влияет на конечный результат генерации.**

Q. Почему это так сложно сделано? Почему мне нельзя самому завести стейт, его менять, вызывать чейндж ивент и смотреть на дефолтное значение?

A. Потому что в разных фреймворках мы генерируем немного разные констуркции (хоть в их сути и лежит одно и то же). А так же эти конструкции позволяют использовать наши компоненты в controlled и uncontrolled режимах.
...
Пользователь просто создал компонент и ему больше ничего не интересно. От каких-то манипуляций с UI конечного пользователя меняется значение `TwoWay` пропа, и комопнент будет перерисовывается.
...
Для нас эти три сценария ничем не отличаются - мы на новом цикле рендера получим то значени, которое записали в `TwoWay` проп.

Это значит, что наш компонент никто не контроллирует, мы сами установили новое значение и его же и получаем.
...
Он контроллирует наш компонент, он решает какое мы значение получим. Хоть мы и записали в проп новое значение, на рендере нам может придти другое значение и мы его и будем рисовать
...
Однако если представить, что вот примеры выше это все тоже наши декларативные компоненты, то чтобы тайпскрипт не ругался и мы в декларативных комопнентах видели, что там есть все такие change ивент и дефолтный проп, их нужно задекларировать.
...
Так же с целью обеспечения [обратной совместимости](#Темплейты) для jQuery виджетов в темплейт необходимо передавать реф на родительский контейнер. При этом парент для темплейта и дефолтного контента (если он предполагается) должен быть одним и тем же.
...
В Ангуляр это Content Projection, в Vue это слоты и т.п.

Слоты могут быть именованными и неименованными. В наших компонентах, тем не менее, все слоты именованные. С одним небольшим нюансом. Если вы отпределите слот с именем `default`, то он будет использовать в соотвествующих технологих на местах неименованного слота - в этот слот попадает весь контент, который не попадает в другие именованные слоты (ну а в Реакте это проп `children`).
...
В DevExtreme есть поддержка анонимных темплейтов, в новом DevExterem стоит использовать слоты.

#### @Effect()

Этим декоратором помечаются методы компонента, которые вызываются после того, как компонент отобразится в дереве. Внтури этих методов можно обращаться к реальному DOM с помощью [рефов](#@Ref()). 

Зачем использовать Эффекты - для обсчета реальных размеров, для оформления всевозможных подписок и так далее. В общем всего того, чот нельзя сделать декларативно. 

Компонент может содержать сколько угодно эффектов. Вообще один эффект (метод) должен быть посвящен чему-то одному. Разные по природе эффекты должны оформляться в разных методах.

Если вы део=лаете некие подписки в эффекте, то Вам необходимо вернуть колбек отписки.

Стоть помнить, что все эффекты вызываются асинхронно, так же в разных фреймворках они и устроены немного по-разному. Поэтому и воспринимать их надо так, что это атомарные дейтсвия, которые не зависят друг от друга и выполняются асинронно (не сразу в момент рендера)
...
Так же `view` функция должна вернуть JSX разметку с одним корневым элементом. Если Ваш компонент должен венуть более одно, их можно обернуть в специальный элемент `Fragment`, который не будет отображен в дереве.
