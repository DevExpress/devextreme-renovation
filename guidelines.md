...
```
`defaultOptionRules` - правила задания дефолтный опций по девайсу (см button.tsx) 
...
Хотя технически компонент записан в виде класс, лучше всего думать о нем, как о функции. Эта функция принимает на вход параметры и выдает разметку. Да, это функциональный реакт-компонент. Наш компонент в реакте представляет из себя именно функциональный компонент.
