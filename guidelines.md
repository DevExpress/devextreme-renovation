...
Для любого `TwoWay` пропа заводится соотвествующий `Event` с `propName`+`Change`, а также `OneWay` проп `default`+`PropName`. Например, для `TwoWay` пропа `pressed` в модели также появятся `Event` `pressedChange` и `OneWay` проп `defaultPressed`.
