...
Для темплейтов, которые будут использоваться только нашими компонентами внутри, вышеуказанные правила можно не применять. Данные можно передавать как угодно (не обязательно `data` и `index`), а `parentRef` задавать вообще не нужно.
