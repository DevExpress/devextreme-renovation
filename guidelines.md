...
Слоты могут быть именованными и неименованными. Однако, в наших компонентах все слоты именованные. С одним небольшим нюансом. Если вы определите слот с именем `default`, то он будет использоваться в соотвествующих технологих на местах неименованного слота - в этот слот попадает весь контент, который не попадает в другие именованные слоты (ну а в Реакте это проп `children`).
...
Стоить помнить, что все эффекты вызываются асинхронно, так же в разных фреймворках они и устроены немного по-разному. Поэтому и воспринимать их надо как атомарные дейтсвия, которые не зависят друг от друга и выполняются асинронно (не сразу в момент рендера), а после.
...
Так же `view` функция должна вернуть JSX разметку с одним корневым элементом. Если Ваш компонент должен венуть более одно, их следует обернуть в специальный элемент `Fragment`, который не будет отображен в дереве.
