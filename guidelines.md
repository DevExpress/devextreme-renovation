...
Однако у них есть одно существенное отличие - компонент может в них писать, менять их значение.

При присваивании нового значения произойдет перерендер компонента, при этом новое значени можно будет считать из этого пропа.
...
А в каких случаях надо их объявить? Если Вы создаете компонент, который хочет подписаться на Change ивент другого компонента, то у этого другого компонента должен быть объявлен этот `Event`. Это нужно только длякоррекной передачи пропов в декларации, на сгенерированный код это никак не влияет. 

То же касается default пропа. Если мне нужно передать не сам проп, а его default собрата, то мне нужно, чтобы он был задекларирован в нужном компоненте. Это нужно только для декларации.

Еще раз. Наличие или отсуствие этих пропов в декларации не влияет на конечный результат генерации.

Q. Почему это так сложно сделано? Почему мне не надо самому завести стейт, его менять, вызывать чейндживент и смотреть на дефолтное значение?
...
Пользователь просто создал компонент и ему больше ничего не интересно. От каких-то действий конечного пользователя мы меняем значение `TwoWay` пропа, комопнент перерисовывается и мы в пропах получаем наше новое значение.
...
Первый сценарий какой-то не очнеь реальный, зачем что-то атм содавать и потом забивать, тем более если оно еще и значение свое меняет.
...
Этот сценарий опять же принципиально не отличается от первых двух. Но тут пользователю компонента нужно еще задать начальное или дефолтное значение. Но на этом все, он больше ничего с ним делать не собирается, ему важно только знать о его изменении.
...
Пользователь компонента указывает (или нет) дефолтное значение, подписывается на изменение, но еще он принимает решение, что же делать с этим измененным значением - его же отдать нам, либо провалидировать и отдать нужное значение (ну например он хочет сбрасывать счетчик после 5).
...
Да, можно, но для других каких-то целей (например для темплейтов). А если вы хоиите правильыне ивенты во всех фреймворках, помечайте такие пропы `Event` декоратором - это обеспечит корректную работу (файринг) ивентов везде.
