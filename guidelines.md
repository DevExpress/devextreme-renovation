...
Все пропы публичных компонентов должны быть необязательными. Для этого нужно задать для пропа интициализатор или сделать его опциональным, используя `?` токен, в этом случае одно из возможных значений пропа будет `undefined`.
