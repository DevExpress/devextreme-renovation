...
Любое изменение `@TwoWay` пропа заставит перерисоваться компонент, а новое значение будет в модели (либо значение, которое вы туда записали, либо другое, если внешний код так решит). Более подробно смотри в [use cases](#@TwoWay())
