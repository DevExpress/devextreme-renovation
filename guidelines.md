...
Любое изменение свойства самого комопнента (не пропов) также вызывает перерисовку, но новое значение сохраняется в этом свойстве.
...
Параметры в темплейт стоит передавать в таком виде:
  ...
  - **parentRef** - *ref* на ноду, в которой будет рисоваться темплейт. *ref* надо объявить, указать в разметке и передать в темплейт.
...
Для темплейтов, которые будут использоваться только нашими компонентами внутри, вышеуказанные правила можно ен применять. Данные можно передавать как угодно (не обязательно `data` и `index`), а `parentRef` задавать вообще не нужно.
...
Также с целью обеспечения [обратной совместимости](#Темплейты) для *jQuery* виджетов в темплейт необходимо передавать *ref* на родительский контейнер. При этом парент для темплейта и дефолтного контента (если он предполагается) должен быть одним и тем же.
