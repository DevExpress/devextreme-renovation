...
function findLastDOMFromVNode(vNode: VNode) {
  ...
  while (vNode) {
    ...
    if (flags & VNodeFlags.Fragment) {
      vNode = (vNode.childFlags as unknown as ChildFlags) === ChildFlags.HasVNodeChildren ? (children as VNode) : (children as VNode[])[children.length - 1];
...
function isSamePropsInnerHTML(dom: Element, props: any): boolean {
