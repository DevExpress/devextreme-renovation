...
cloneTest("Click on list item", async (t) => {
  const el = Selector("#list-1 .list-item");

  const count = await el.count;

  await t.click(el);

  const changedElements = Selector("#list-1 .list-item");
  ...
  const readyCounter = Selector("#list-1 .ready-counter");

  await t.expect(await readyCounter.textContent).eql("6");
});

cloneTest("Click on template list item", async (t) => {
  const el = Selector("#list-2 .list-item");

  const count = await el.count;

  await t.click(el);

  const changedElements = Selector("#list-2 .list-item");
  const changedCount = await changedElements.count;
  ...
  const readyCounter = Selector("#list-2 .ready-counter");
...
cloneTest("Context - share object", async (t) => {
  const checkValue = async (expected) => {
    const pagerValueEl = Selector("#pager span.value");
    const pagingEl = Selector("#context-paging-value");
    const appEl = Selector("#context-page-selector span.value");
    const getterContext = Selector("#context-getter-provider");
...
cloneTest("Set forward ref", async (t) => {
  const content = Selector("#set-ref");
...
cloneTest("Dynamic components", async (t) => {
  const dynamicComponent = Selector("#dynamic-component");
  const dynamicComponentCondition = Selector(
    "#dynamic-component-condition"
  );
  const dynamicComponentArray = Selector(".dynamic-component-array");
  const dynamicComponentSlot = Selector("#dynamic-component-slot");
  const dynamicComponentWithTemplate = Selector(
