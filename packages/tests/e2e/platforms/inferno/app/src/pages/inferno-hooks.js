...
import { ConfigContext } from "./testing-button/common/widget";
...
function Test(props, ref) {
  return <HookContainer renderFn={
    () => {
      ...
      return <div ref={ref1} onClick={onClick}/*onclick={() => { setState(state + 1) }}*/>
        {text}
        <div>{depState}</div>
...
export default class NativeComponents extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <ConfigContext.Provider value={{ rtlEnabled: this.state.rtlEnabled }}>
          ...
          {/* { array10000.map(()=><InfernoButtonFR  {...Button.defaultProps}>InfernoButton</InfernoButtonFR>)} */}
          { array10000.map(()=><InfernoButtonFR  {...Button.defaultProps} onClick={log123}>{this.state.buttonChildren || "InfernoButton"}</InfernoButtonFR>)}
          {/* { array10000.map(()=><InfernoButtonFR  {...Button.defaultProps} template={this.state.buttonTemplate}>Inferno Button</InfernoButtonFR>)} */}
          {/* {array10000.map((value, index)=>{return <InfernoButtonFR  {...Button.defaultProps} visible={this.state.visible}>Inferno Button</InfernoButtonFR>})} */}
          {/* {array10000.map((value, index)=>{return <InfernoButtonFR ref={ref10000[index]}{...Button.defaultProps} >InfernoButton</InfernoButtonFR>})} */}
          {/* {this.state.enable ?  array10000.map(()=><InfernoButtonFR  {...Button.defaultProps}>InfernoButton</InfernoButtonFR>) : null} */}
          {/* {array10000.map((value, index)=>{return <InfernoButtonFR  {...Button.defaultProps} onClick={this.state.onButtonClick} disabled={this.state.visible}>Inferno Button</InfernoButtonFR>})} */}
          {/* {this.state.enable ? array10000.map((value, index) => { return <InfernoButtonFR  {...Button.defaultProps} onClick={this.state.onButtonClick} disabled={this.state.disabled}>Inferno Button</InfernoButtonFR> }):null} */}
          {/* {array10000.map((value, index)=>{return <InfernoButtonFR  {...Button.defaultProps}>Inferno Button</InfernoButtonFR>})} */}
